...
% --- Additional packages for pedagogical enrichment ---
\usepackage{xcolor}
\usepackage{listings}
...
% --- Ethical disclaimer ---
\begin{ethique}
The techniques presented in this lab are studied within a \textbf{strictly academic context} in order to understand the mechanisms of memory vulnerability exploitation and thus be able to better protect against them.

\textbf{Any use of these techniques outside the educational context is illegal} and punishable under criminal law. Under French law (Chapter III of the Penal Code, articles 323-1 to 323-8 -- \textit{Offences against automated data processing systems}):
\begin{itemize}
    \item Unauthorized access to or remaining in an automated data processing system (art.~323-1): \textbf{3 years} imprisonment and a fine of \textbf{\texteuro{}100,000}
    \item Obstructing the operation of an automated data processing system (art.~323-2): \textbf{5 years} imprisonment and a fine of \textbf{\texteuro{}150,000}
    \item Fraudulent introduction of data into an automated data processing system (art.~323-3): \textbf{5 years} imprisonment and a fine of \textbf{\texteuro{}150,000}
\end{itemize}
Penalties are increased to \textbf{7 years} and \textbf{\texteuro{}300,000} when the targeted system processes personal data operated by the State, and up to \textbf{10 years} and \textbf{\texteuro{}300,000} for organized crime (art.~323-4-1). \textit{Penalties updated by the LOPMI Act no.~2023-22 of January 24, 2023.}
\end{ethique}
...
\medskip
\noindent\textbf{Required environment:} x86-64 Linux machine (Debian 12/13 recommended). Tools: \texttt{gcc}, \texttt{gdb}, \texttt{python3}, \texttt{objdump}, \texttt{ROPgadget} (optional).

\bigskip

\begin{attention}
All experiments must be performed on a \textbf{dedicated virtual machine} or an isolated environment. Disable ASLR \textbf{only} in this environment:
...
In x86-64 architecture, the \textbf{stack} grows toward lower addresses. The main registers are:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \texttt{run < <(python3 -c "...")}: run with input generated by Python
\end{itemize}
\end{rappel}
...
\end{lstlisting}
Run the program with a normal input (e.g., \texttt{AAAA}). Observe the addresses displayed by the \texttt{[DEBUG]} messages.

\item[\textbf{1b)}] Open the program in GDB (\texttt{gdb ./stackx64}). Disassemble the function \texttt{vulnerable\_function} with \texttt{disassemble vulnerable\_function}. Identify the instruction \texttt{sub rsp, 0x...} that reserves space for local variables. What is the reserved size?
...
\begin{itemize}
    \item The address and contents of the buffer
    \item The address and value of the saved RBP
    \item The address and value of the return address
\end{itemize}

\item[\textbf{1d)}] Calculate the \textbf{offset} (distance in bytes) between the beginning of the buffer and the return address. Verify your calculation with GDB by sending a string of known size and observing the overwrite.
...
\begin{itemize}
    ...
    \item In x86-64, addresses are 8 bytes wide but user-space addresses only use the lower 6 bytes (the upper 2 bytes are \texttt{0x00}).
    \item The return address is located at \texttt{RBP + 8} (just above the saved RBP).
\end{itemize}
\end{pointcle}
...
A \textbf{shellcode} is a sequence of bytes representing directly executable machine code, designed to be injected into memory. Its classic goal is to open a \textit{shell} (\texttt{/bin/sh}).
...
\medskip
\textbf{Null byte constraint:} C string handling functions (\texttt{strcpy}, \texttt{gets}, etc.) stop at the first \texttt{0x00} byte. Therefore, the shellcode must contain \textbf{no null bytes}.
\end{rappel}
...
push   rsp                    ; push the stack top address
...
\item[\textbf{2c)}] Explain the concept of the \textbf{NOP sled} (\texttt{0x90} = \texttt{NOP} instruction). Draw a diagram showing why we aim at the middle of the NOP sled rather than exactly at the beginning of the shellcode.
...
\begin{enumerate}
    ...
    \item \texttt{push rsp}: pushes the stack top address (which points to \texttt{"/bin//sh"}).
...
\textbf{2b)} \texttt{mov rsi, 0} is encoded as \texttt{48 c7 c6 \textbf{00 00 00 00}}: it contains \textbf{null bytes} (\texttt{0x00}). Functions like \texttt{gets()}, \texttt{strcpy()} interpret \texttt{0x00} as a string terminator and stop copying. The shellcode would therefore be \textbf{truncated}.

\texttt{xor rsi, rsi} is encoded as \texttt{48 31 f6} (3 bytes, no null bytes) and produces the same result: $x \oplus x = 0$.

\textbf{2c)} The NOP sled is a zone of \texttt{NOP} (\texttt{0x90}) instructions placed before the shellcode:
\begin{lstlisting}[style=GDBStyle]
+------------------------------------------+
...
to the shellcode without error.
\end{lstlisting}

The exact buffer address can vary slightly between executions (environment variables, arguments). By aiming at the \textbf{middle} of the NOP sled, we tolerate an error of $\pm$(sled size / 2) bytes.

\textbf{2d)} The program \texttt{test\_shellcode.c} allocates the shellcode in a global variable (\texttt{.data} section), casts it to a function pointer and calls it. With \texttt{-z execstack} (which also makes data executable on certain configurations), the shellcode executes and opens an interactive \texttt{/bin/sh} shell.
...
\begin{pointcle}
\begin{itemize}
    \item \textbf{Null bytes} (\texttt{0x00}) are the enemy of shellcode: they prematurely terminate the copy by standard C functions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PHASE 3: Shellcode injection via overflow (~35 min)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Phase 3 -- Shellcode injection via overflow \hfill\normalfont\textit{($\sim$35 min)}}

\begin{rappel}
\textbf{Structure of a shellcode injection payload:}

The payload is constructed to fit exactly within the buffer + RBP + return address space:
...
\begin{enumerate}
\item[\textbf{3a)}] In GDB, find the exact address of the buffer. Set a breakpoint just after the \texttt{gets}, run with any input, then examine the address displayed by the program or read the RSP register.

\item[\textbf{3b)}] Build the complete Python3 payload:
\begin{itemize}
...
\begin{attention}
Addresses may be \textbf{different} between GDB and direct execution. GDB adds environment variables that shift the stack. If the exploit works in GDB but not outside, adjust the address (typically a few dozen bytes of difference).
\end{attention}
...
\begin{pointcle}
\begin{itemize}
    \item Shellcode injection grants the \textbf{same privileges} as the vulnerable program. If the program is \texttt{setuid root}, the attacker gets a root shell.
...
The \textbf{NX} protection marks the stack (and other data regions) as \textbf{non-executable}. Any attempt to execute code on the stack causes a \texttt{SIGSEGV} signal.
...
The x86-64 ABI requires that RSP be aligned to 16 bytes during a \texttt{call}. After a \texttt{ret} (which pops 8 bytes), RSP is no longer aligned. An additional \texttt{ret} gadget realigns the stack.
...
\end{lstlisting}
Re-run the Phase 3 exploit. What happens and why?
...
\end{lstlisting}
Then add the found offset to the libc base address (visible with \texttt{info proc mappings} in GDB).
...
\item[\textbf{4f)}] Build the complete ret2libc payload in Python3 and run the exploit.

\item[\textbf{4g)}] Fill in the comparative table between shellcode injection (Phase 3) and ret2libc (Phase 4):
\begin{center}
\begin{tabular}{lcc}
\toprule
...
\textbf{4d)} \texttt{pop rdi; ret} gadget:
\begin{lstlisting}[style=GDBStyle]
$ ROPgadget --binary stackx64 | grep "pop rdi"
0x0000000000401283 : pop rdi ; ret
\end{lstlisting}

\textbf{4e)} \texttt{ret} gadget (for alignment):
\begin{lstlisting}[style=GDBStyle]
...
\textbf{4g)} Completed comparative table:
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Criterion} & \textbf{Shellcode injection} & \textbf{Return-to-libc} \\
...
Sensitive to libc ASLR? & No (code on the stack) & Yes \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{16-byte stack alignment} is mandatory on x86-64: an additional \texttt{ret} gadget is often necessary.
    \item Ret2libc demonstrates that a single protection is not enough: \textbf{defense in depth} is required (NX + ASLR + canaries + ...).
\end{itemize}
\end{pointcle}
...
\medskip
\textbf{Direct access with \texttt{\$}:} The format \texttt{\%k\$hhn} uses the \textbf{k-th} stack argument for writing, without consuming previous arguments.
...
\end{lstlisting}
The \texttt{-U\_FORTIFY\_SOURCE} option disables the protection that blocks \texttt{\%n} in recent versions of the glibc.
\end{attention}
...
\item[\textbf{5c)}] Build the \texttt{\%hhn} exploit to write a non-zero value to \texttt{is\_admin}. The strategy is:
\begin{enumerate}
    \item Place the address of \texttt{is\_admin} at the end of the payload (after the format specifiers)
    \item Use padding to adjust the number of printed characters
    \item Use \texttt{\%hhn} with the appropriate offset to write to \texttt{is\_admin}
\end{enumerate}

\item[\textbf{5d)}] Run the exploit and verify that the message \texttt{"Admin access granted!"} is displayed.
\end{enumerate}
...
\textbf{5c)} Building the exploit. The address \texttt{0x404040} contains null bytes (\texttt{0x00 0x00 0x40 0x40 0x40 0x00 0x00 0x00}). We place it at the \textbf{end of the string}. We must calculate the position of this address on the stack.

The buffer starts at stack offset 6. If our format string is 16 bytes before the address (2 quadwords), the address will be at offset $6 + 2 = 8$.
...
\textbf{5d)} The message \texttt{"Admin access granted!"} is displayed because \texttt{is\_admin} is now 2 (the number of characters \texttt{"AA"} printed before the \texttt{\%hhn}), which is non-zero $\Rightarrow$ the condition \texttt{if(is\_admin)} is true.

\begin{attention}
The exact offset depends on the length of the format string and stack alignment. It often requires trial and error. Use GDB to verify the position of the address on the stack.
\end{attention}

\begin{pointcle}
\begin{itemize}
    \item \texttt{\%n} transforms a \textbf{read} vulnerability (classic format string) into an \textbf{arbitrary write} vulnerability.
    \item In 64-bit, addresses contain null bytes $\Rightarrow$ the address is placed \textbf{at the end of the string} to avoid truncation.
...
\begin{itemize}
    ...
    \item The \textbf{flag} (compilation or system) to enable/disable it
\end{itemize}
...
\item[\textbf{6c)}] What \textbf{minimal combination} of protections blocks all 4 types of attacks covered in this lab?
...
\item \textbf{NX / DEP}: Memory pages are marked with a NX (No-eXecute) bit. The stack, heap, and data segments are marked non-executable. The processor refuses to execute code located in these regions. \textbf{Blocks} shellcode injection on the stack or heap. \textbf{Flags}: \texttt{-z execstack} (disables) / enabled by default.

\item \textbf{ASLR} (\textit{Address Space Layout Randomization}): The kernel randomizes the addresses of the stack, heap, libc, and shared libraries at each execution. The attacker can no longer predict addresses. \textbf{Blocks} (partially) attacks that require known addresses (shellcode injection, ret2libc). \textbf{Command}: \texttt{sysctl kernel.randomize\_va\_space=2} (enable) / \texttt{=0} (disable).

\item \textbf{PIE} (\textit{Position-Independent Executable}): The executable code itself is compiled to be loaded at a random address (complementing ASLR which randomizes libraries). The attacker can no longer rely on fixed addresses in the binary (gadgets, functions). \textbf{Flags}: \texttt{-pie} / \texttt{-no-pie}.
...
\begin{itemize}
    \item \textbf{ASLR / shellcode injection}: the stack address is randomized, but a very large NOP sled could compensate (brute-force attack). This is impractical on 64-bit (address space too large) but theoretically possible.
...
\begin{itemize}
    ...
    \item Format string \texttt{\%n}: writing is harder with \textbf{ASLR} (unknown target addresses), but canaries do not directly protect. However, with ASLR + PIE, global variable addresses are also randomized.
\end{itemize}
...
\begin{itemize}
    ...
    \item Modern compilers (\texttt{gcc}, \texttt{clang}) enable \textbf{all} of these protections by default (stack canaries, NX, ASLR, PIE, partial RELRO). The developer must explicitly disable them (as in this lab) to allow attacks.
\end{itemize}

\begin{pointcle}
\begin{itemize}
    \item \textbf{No single protection is sufficient}: stack canaries do not block format strings, NX does not block ret2libc, ASLR can be bypassed through information leaks.
    \item \textbf{Defense in depth} forces the attacker to bypass multiple mechanisms simultaneously, greatly increasing the difficulty.
    \item Modern compilers enable \textbf{all} of these protections by default. In this lab, we disabled them one by one to understand their role.
